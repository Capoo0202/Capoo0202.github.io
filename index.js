...
function sumofchooseselect(x) {
	...
	select1[x].options.remove(101);
	var sum = 1000 * select1[x].value * select3[x].value;
	var opt = new Option(sum, sum);
...
function unitofchooseselect(x) {
	...
	if ( select2[x].value == 0 ) select1[x].value = 0;
		...
		var opt = new Option(unit.toFixed(2), unit.toFixed(2));
		select1[x].options.add(opt);
		select1[x].value = unit.toFixed(2);	
	}
